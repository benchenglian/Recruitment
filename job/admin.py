...
class JobAdmin(admin.ModelAdmin):
    ...
    exclude = ('creator','created_date','modifled_date')
    list_display = ('job_name','job_type','creator','created_date','modifled_date')
...
class ResumeAdmin(admin.ModelAdmin):
    ...
    def save_model(self, request, obj, form, change):
        obj.applicant = request.user # 申请人保存，当前登录用户。
        super().save_model(request, obj, form, change)

admin.site.register(Job,JobAdmin)# '@timestamp': {'gt': 'now-1m'}
